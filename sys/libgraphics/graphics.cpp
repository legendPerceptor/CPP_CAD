...
typedef struct graphicsStateTa {
    double cx, cy;
    char* font;
    int size;
    int style;
...
static char* windowTitle = (char*)"Graphics Window";
...
static void RenderArc(double x, double y, double rx, double ry,
                      double start, double sweep);
static void DisplayText(double x, double y, char* text);
static void DisplayFont(char* font, int size, int style);
static int FindExistingFont(char* name, int size, int style);
...
static void SetTextBB(RECT *rp, double x, double y, char* text);
...
static int FindColorName(char* name);

static BOOL StringMatch(char* s1, char* s2);
...
void DrawLine(double dx, double dy)
{
    ...
    switch (regionState) {
      ...
      case RegionStarting: case RegionActive:
        ...
        break;
      case PenHasMoved:
        Error((char*)"Region segments must be contiguous");
    }
    cx += dx;
    cy += dy;
}
...
void StartFilledRegion(double grayScale)
{
    ...
    if (regionState != NoRegion) {
        Error((char*)"Region is already in progress");
    }
    if (grayScale < 0 || grayScale > 1) {
        Error((char*)"Gray scale for regions must be between 0 and 1");
    }
...
void EndFilledRegion(void)
{
    InitCheck();
    if (regionState == NoRegion) {
        Error((char*)"EndFilledRegion without StartFilledRegion");
    }
    DisplayPolygon();
...
void DrawTextString(char* text)
{
    InitCheck();
    if (regionState != NoRegion) {
        Error((char*)"Text Strings are illegal inside a region");
    }
...
double TextStringHeight(char* text)
{
    RECT r;

    InitCheck();
    SetTextBB(&r, cx, cy, text);
    return (InchesY(RectHeight(&r)));
}
...
void SetPenColor(char* color)
{
    ...
    if (cindex == -1) Error((char*)"Undefined color: %s", color);
    penColor = cindex;
}
                 ...
                 double red, double green, double blue)
{
    ...
    if (red < 0 || red > 1 || green < 0 || green > 1 || blue < 0 || blue > 1) {
        Error((char*)"DefineColor: All color intensities must be between 0 and 1");
    }
    cindex = FindColorName(name);
    if (cindex == -1) {
        if (nColors == MaxColors) Error((char*)"DefineColor: Too many colors");
        cindex = nColors++;
    }
...
void RestoreGraphicsState(void)
{
    ...
    InitCheck();
    if (stateStack == NULL) {
        Error((char*)"RestoreGraphicsState called before SaveGraphicsState");
    }
    sb = stateStack;
    cx = sb->cx;
    cy = sb->cy;
...
static void InitCheck(void)
{
    if (!initialized) Error((char*)"InitGraphics has not been called");
}
...
/*    RegisterWindowClass();*/
    ...
    if (osdc == NULL) {
        Error((char*)"Internal error: Can't create offscreen device");
    }
    osBits = CreateCompatibleBitmap(gdc, pixelWidth, pixelHeight);
    if (osBits == NULL) {
        Error((char*)"Internal error: Can't create offscreen bitmap");
    }
...
static void InitDrawingTools(void)
{
    ...
    if (drawPen == NULL || erasePen == NULL || nullPen == NULL) {
        Error((char*)"Internal error: Can't initialize pens");
    }
    for (i = 0; i < NFills; i++) {
...
static void RegisterWindowClass(void)
{
    ...
    if (!RegisterClass(&wcApp)) {
        Error((char*)"Internal error: RegisterClass failed\n");
    }
}
...
static void DisplayFont(char* font, int size, int style)
{
    ...
    for (i = 0; (fontBuffer[i] =(char) tolower(font[i])) != '\0'; i++);
    if (StringEqual((char*)"default", fontBuffer)) {
        fontName =(char*) DefaultFont;
    } else {
        fontName = fontBuffer;
    }
    ...
    if (fontIndex == -1) {
        ...
        if (newFont != NULL) {
            ...
            if (PrefixMatch(fontName, faceName)
                ...
                if (nFonts == MaxFonts) Error((char*)"Too many fonts loaded");
                fontIndex = nFonts++;
...
/*void SetTextRegion(RECT *rp, double x, double y,String text){
    SetTextBB(rp,x,y,text);
}*/
/*
