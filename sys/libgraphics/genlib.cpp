...
void *GetBlock(size_t nbytes)
{
    ...
    if (result == NULL) Error((char*)"No memory available");
    return (result);
}
...
void Error(char* msg, ...)
{
    ...
    if (_acb == NULL) {
        errmsg = (char*)malloc(errlen + 1);
    } else {
        errmsg =(char*) _acb->allocMethod(errlen + 1);
    }
    if (errmsg == NULL) {
        errmsg = (char*)"No memory available";
    } else {
        strcpy(errmsg, errbuf);
    }
    if (HandlerExists(&ErrorException)) {
        RaiseException(&ErrorException, (char*)"ErrorException", errmsg);
    } else {
